...
## 47. [2026-09-04] 댓글 작성 폼에서 홈페이지 입력 제거.

[2026-09-04] 댓글 작성 폼에서 홈페이지 입력 제거.
게스트 필드는 이름([##_rp_input_name_##])·비밀번호([##_rp_input_password_##])만.
[##_rp_input_homepage_##] / [##_guest_homepage_##]는 마크업에 두지 않는다.

---

## 48. 대표이미지 — 목록의 post-thumb과 같은 조건부 블록.

대표이미지 — 목록의 post-thumb과 같은 조건부 블록.
                         없는 글은 서버가 이 블록을 통째로 빼므로 빈 자리가 남지 않는다.

---

## 49. [FOOTER SPEC §1-1] 글 상세 하단 4종.

[FOOTER SPEC §1-1] 글 상세 하단 4종.
                         post-single **안쪽**에 둔다 — 형제로 두면 720px max-width를
                         한 번 더 복제해야 하고 두 값이 어긋나면 본문과 댓글의 좌측선이
                         틀어진다. post-single-body의 형제이므로 §10 프로즈 자손
                         셀렉터는 여기로 상속되지 않는다.
...
## 51. 공감 — 서버 API도 스킨 치환자도 없어 localStorage 로컬

공감 — 서버 API도 스킨 치환자도 없어 localStorage 로컬
                               카운터다. 숫자는 liked?1:0이며 다른 방문자의 공감은
                               알 방법이 없다 — 가짜 수치를 만들지 않는다(§3-3 / Q2).

---

## 52. 댓글 수 — 서버 데이터. 클릭 동작 없음(§3-1)

댓글 수 — 서버 데이터. 클릭 동작 없음(§3-1)

---

## 53. 공유 — Web Share API. hidden이 기본값인 이유: navigator.share가

공유 — Web Share API. hidden이 기본값인 이유: navigator.share가
                               없는 데스크톱에서 눌러도 아무 일이 없는 버튼을 보여주지 않기
                               위해서다. JS가 지원을 확인하면 hidden을 뗀다. JS가 아예 안 돌면
                               계속 안 보이고 링크 복사만 남는다 — 안전한 실패.
...
## 55. 관리자에게만 서버가 렌더한다(§0-4). 스크린샷의 더보기(⋯)

관리자에게만 서버가 렌더한다(§0-4). 스크린샷의 더보기(⋯)
                               드롭다운 대신 이 인라인 버튼들로 대체했다 — Popover
                               프리미티브와 popover 색 토큰이 없고, ⋯에 넣을 데이터
                               소스도 우리에게 없다(§3-1 / Q1).
...
② '카테고리' 카테고리의 다른 글
                           ★ 반복 블록 <s_article_related_rep>가 있다(§0-1) — 이걸
                             빠뜨리면 관련글이 딱 1개만 나온다. 썸네일 조건부 블록은
                             마크업에 아예 넣지 않는다(넣고 CSS로 숨기면 서버가
                             이미지를 다운로드한다 — §5-1 / Q5).
                           목록은 widgets.css의 widget-* 프리미티브를 그대로 재사용한다.
...
③ 이 글의 태그
                           ★ [##_tag_label_rep_##]는 반복 블록이 아니라 태그 링크 묶음
                             전체를 통째로 내려주는 **단일 치환자**다(§0-2). 태그 하나하나에
                             data-slot="badge"를 붙일 마크업 자리가 구조적으로 없어
                             content.js의 initPostTags()가 정규화한다(CSS 폴백 있음).
                             사이드바 위젯의 <s_random_tags>(블로그 전체 태그 클라우드)와는
                             작동 방식이 완전히 다르다 — 같은 방식으로 만들면 안 된다.

---

## 58. ④ 댓글 — [##_comment_group_##](티스토리 자동 렌더) 대신

④ 댓글 — [##_comment_group_##](티스토리 자동 렌더) 대신
                           완전 수동 마크업. 이 프로젝트가 모든 Tistory 기능을 우리
                           디자인 시스템으로 직접 마크업해 온 선례를 따른다.

---

## 59. fallback을 배경으로 깔고 [##_rp_rep_logo_##]가 그 위를

fallback을 배경으로 깔고 [##_rp_rep_logo_##]가 그 위를
                                       덮는다 — Radix의 이미지 실패 감지 런타임이 없는 대신
                                       같은 결과를 만드는 CSS 레이어 방식(§8-6).

---

## 60. 대댓글에는 onclick_reply를 넣지 않는다 —

대댓글에는 onclick_reply를 넣지 않는다 —
                                                     공식 예제가 그렇고(2단계까지만 지원),
                                                     있지도 않은 치환자를 쓰면 빈 onclick이 된다.

---

## 61. ★ [##_rp_input_*_##]는 href나 요소 전체가 아니라 name

★ [##_rp_input_*_##]는 href나 요소 전체가 아니라 name
                                   **속성의 값**이다. 이 형태를 글자 그대로 유지해야
                                   서버가 값을 받는다 — 임의로 name="comment" 같은 걸
                                   쓰면 댓글이 등록되지 않는다(§0-3).
                                   로그인 분기는 서버가 한다: 양쪽 마크업을 항상 써 두고
                                   <s_rp_member>/<s_rp_guest>의 판단을 서버에 맡긴다.

---

## 62. value의 [##_rp_admin_check_##]는 관리자 자동입력 값이다.

value의 [##_rp_admin_check_##]는 관리자 자동입력 값이다.
                                         빼면 관리자가 댓글 쓸 때마다 비번을 치게 된다 — 반드시 유지.

---

## 63. 비밀글 — 네이티브 checkbox를 .sr-only로 숨기고 label을

비밀글 — 네이티브 checkbox를 .sr-only로 숨기고 label을
                                       자물쇠 아이콘 버튼처럼 그린다. :has(:checked)로 순수 CSS
                                       토글이라 JS가 0줄이고, 접근성(Tab·Space)은 실제
                                       체크박스가 그대로 담당한다.

---

## 64. 공식 예제의 <input type="submit">을 <button type="button">으로

공식 예제의 <input type="submit">을 <button type="button">으로
                                       바꿨다 — 감싸는 <form>이 없어 submit 타입이 의미가 없고
                                       (오히려 상위에 폼이 생기면 원치 않는 제출이 발생),
                                       아이콘+텍스트 버튼을 쓰려면 <button>이어야 한다.
                                       onclick 치환자는 글자 그대로 유지(§8-7).
