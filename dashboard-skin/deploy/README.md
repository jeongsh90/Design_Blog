...
**⚠ 기존 JS를 재업로드(덮어쓰기)했는데 실사이트 동작이 안 바뀐다면(2026-09-07
실측):** `skin.html`이 참조하는 `<script src="…/category.js?_version_=
{숫자}">`의 그 숫자는 파일 업로드 시점이 아니라 **HTML/CSS 탭에서 마지막으로
"적용"을 누른 시점**에 고정된다 — 파일업로드 탭만 단독으로 다시 올리면 CDN이
옛 버전 번호로 캐시된 옛 내용을 계속 서빙할 수 있다(버전 없는 원본 경로는
새 내용을 정상 서빙하는데도!). 재업로드 후 실사이트에서 실제 동작을 반드시
확인하고, 옛 버전이 계속 나오면 HTML 탭에서 의미 없는 사소한 변경(파일 끝에
빈 줄 추가 등)으로 "적용"을 한 번 더 눌러 새 버전 번호를 발급받는다.
