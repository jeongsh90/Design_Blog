...
## 2026-09-11 — `initPostTags` 목록 카드 복수 지원

상세 페이지의 `post-tags-list` 하나뿐 아니라 목록 카드마다 있는 `post-tags-list`도 badge로 정규화하도록 `querySelectorAll` + `normalizePostTagsList`로 바꿨다.
