...
**[같은 날 §12로 대체됨]** 이 "첫 번째 항목 고정" 로직은 이후 "방문
중인 페이지가 속한 카테고리를 연다"로, 일치하는 게 없을 때의 폴백은
"아무것도 열지 않는다"로 바뀌었다 — 아래 §12 참고.
...
안 겹치는 페이지)는 처음엔 §11의 `firstExpandableIndex`로 폴백했었다 —
"아무것도 안 열려 있는 사이드바"보다는 "뭔가는 열려 있는" 편이 낫다고
판단했었는데, **"Ai처럼 하위메뉴가 없을 시 클릭하면 첫번째 리스트가
열려있음" 지적으로 이 폴백 자체가 틀렸다는 게 확인됐다** — 리프
카테고리(Ai)를 보고 있는데 전혀 무관한 Design이 열려 있는 게 오히려
"이 페이지가 Design과 관련 있다"는 잘못된 신호였다. 폴백을 완전히
제거 — 일치하는 확장형 카테고리가 없으면 `openIndex`가 `-1`로 남아
아무 카테고리도 기본으로 열리지 않는다(전부 접힌 채 시작). `initActive
State()`(어떤 링크에 `data-active="true"`를 붙일지)와 이 로직은 서로
다른 함수·다른 계산이지만 같은 `here` 정규화 방식을 쓰므로 결과가 항상
서로 들어맞는다 — "Font 링크가 활성 표시됐는데 그 부모 Design은 접혀
있다" 같은 불일치가 날 수 없다.
