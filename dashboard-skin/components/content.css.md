...
## 2026-09-11 — 목록 카드 태그 스타일

`post-item-inner` 선택자에서 `a` 접두를 제거(마크업이 div로 바뀜). `post-item-hitbox`는 absolute inset 커버. `post-item-tags`는 z-index:1로 히트박스 위. 태그가 있는 카드는 요약을 1줄로 줄여(`:has`) 격자 높이 안에서 배지가 잘리지 않게 했다. thumb 뷰에서는 카드 높이가 auto라 태그 줄이 자연스럽게 요약 아래에 붙는다.

---

## 2026-09-16 — post-item-tags 상단 여백

`post-body`가 `gap: calc(var(--spacing) * 1.5)`(6px)인 flex column이라, 태그 줄도 메타/제목/요약과 똑같은 간격으로만 붙어 있어 요약과 너무 붙어 보였다. `post-item-tags`에 `margin-top`을 더해 요약과의 간격만 별도로 넓힘 — 위쪽 메타↔제목↔요약 간격(6px, thumb 뷰는 4px)은 그대로 유지.

처음엔 `calc(var(--spacing) * 1)`(4px, 합쳐서 총 8~10px)로 넣었다가, 여전히 붙어 보인다는 피드백으로 `calc(var(--spacing) * 3)`(12px, 합쳐서 총 16~18px)까지 올렸다.
