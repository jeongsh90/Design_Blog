# 제목: 연성체 무료 한글 웹폰트 — 여린 손글씨 느낌의 바탕체

- 카테고리: Design > Font
- 태그: 연성체, 무료폰트, 한글폰트, 바탕체, 웹폰트
- 메타디스크립션: 배달의민족(㈜우아한형제들)이 배포하는 무료 한글 폰트 연성체를 실제 페이지 기준으로 검증해 라이선스와 웹폰트 코드까지 정리했습니다.
...
## 검증 기록
- 1차 출처: noonnu.cc 연성체 페이지, 우아한형제들 폰트 사이트(font.woowahan.com)
- 확인 시점: 2026-09-07
- 직접 검증한 항목: @font-face 코드는 Playwright로 noonnu 페이지를 직접 열어 `document.documentElement.innerHTML`에서 정규식으로 실측 확인(WebFetch 미사용) — `font-family: 'Yeonseong'; src: url('https://cdn.jsdelivr.net/gh/projectnoonnu/noonfonts_one@1.0/BMYEONSUNG.woff') format('woff');`(페이지 하단 "웹폰트로 사용" 섹션에 실제 노출된 family명 사용). 배달의민족 폰트 라이선스 본문(OFL 1.1)과 요약표 원문 대조.
