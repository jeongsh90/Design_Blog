# 제목: 위드라이브 전용서체 무료 한글 웹폰트 — 위드라이브 전용서체의 결을 담은 무료 한글 서체

- 카테고리: Design > Font
- 태그: 위드라이브전용서체, 무료폰트, 한글폰트, 고딕체, 웹폰트
- 메타디스크립션: 위드라이브 X 얼리폰트이 배포하는 무료 한글 폰트 위드라이브 전용서체를 실제 페이지 기준으로 검증해 라이선스와 웹폰트 코드까지 정리했습니다.
...
## 검증 기록
- 1차 출처: font_page/963 (위드라이브 전용서체)
- 확인 시점: 2026-09-10
