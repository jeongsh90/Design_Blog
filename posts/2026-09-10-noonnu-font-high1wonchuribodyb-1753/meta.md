# 제목: 하이원 원추리체 무료 한글 웹폰트 — 하이원 원추리체의 결을 담은 무료 한글 서체

- 카테고리: Design > Font
- 태그: 하이원원추리체, 무료폰트, 한글폰트, 고딕체, 웹폰트
- 메타디스크립션: 강원랜드이 배포하는 무료 한글 폰트 하이원 원추리체를 실제 페이지 기준으로 검증해 라이선스와 웹폰트 코드까지 정리했습니다.
