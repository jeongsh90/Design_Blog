# 제목: 와구리체 무료 한글 웹폰트 — 와구리체의 결을 담은 무료 한글 서체

- 카테고리: Design > Font
- 태그: 와구리체, 무료폰트, 한글폰트, 고딕체, 웹폰트
- 메타디스크립션: 구리시 X 투게더그룹이 배포하는 무료 한글 폰트 와구리체를 실제 페이지 기준으로 검증해 라이선스와 웹폰트 코드까지 정리했습니다.
